//import { region_comuna } from "../js/region_comuna.js";
...
const formularioPrincipal = document.getElementById("form_agregar_actividad");
console.log("Formulario Principal encontrado:", formularioPrincipal);
...
const poblarComunas = () => {
  ...
  const comunasFiltradas = todasLasComunasDesdeServidor.filter(
    (comuna) => comuna.region_id === parseInt(region_value)
  );
  comunasFiltradas.forEach((comuna) => {
    let option = document.createElement("option");
    option.value = comuna.id; // Usamos el ID de la comuna para el valor
    option.text = comuna.nombre; // Y el nombre para el texto visible
